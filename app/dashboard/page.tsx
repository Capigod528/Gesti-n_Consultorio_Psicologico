...
import { TrendingUp, Users, Calendar, ArrowUpRight, Sparkles } from "lucide-react";
...
export default function DashboardPage() {
  ...
  if (status === "loading") {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC]">
        <p className="font-bold text-slate-400 animate-pulse">Cargando panel profesional...</p>
      </div>
    );
  }
  ...
  // Anderson, usamos el nombre de la sesión o el que definimos por defecto
  const userName = session.user?.name || "Ander";
    ...
    <div className="p-8 bg-[#F8FAFC] min-h-screen font-sans">
      <header className="mb-10">
        ...
        <p className="text-slate-500 font-medium mt-1 flex items-center gap-2">
          ...
          Aquí tienes un resumen de la clínica para hoy.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
        <div className="bg-white p-7 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-50 flex items-center justify-between transition-transform hover:-translate-y-1">
          <div>
            <p className="text-slate-400 text-xs font-black uppercase tracking-widest">Pacientes Totales</p>
            <h3 className="text-3xl font-black text-slate-900 mt-1">124</h3>
          </div>
        ...
        <div className="bg-white p-7 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-50 flex items-center justify-between transition-transform hover:-translate-y-1">
          <div>
            <p className="text-slate-400 text-xs font-black uppercase tracking-widest">Rendimiento</p>
      ...
      <div className="bg-indigo-600 rounded-[3rem] p-12 text-white flex flex-col md:flex-row justify-between items-center shadow-2xl shadow-indigo-200 relative overflow-hidden group">
        <div className="relative z-10 text-center md:text-left">
          ...
          <p className="text-indigo-100 font-medium text-lg max-w-md">
            Revisa la agenda de citas para organizar tus sesiones de forma profesional.
          </p>
        </div>
        ...
        >
          Ver Agenda <ArrowUpRight size={22} strokeWidth={3} />
        </button>
